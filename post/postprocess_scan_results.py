...
def find_candidates(logdir, tol=1e-8):
    ...
    for fn in sorted(os.listdir(logdir)):
        ...
        with open(path, 'r', errors='ignore') as f:
            txt = f.read()
        # try to find last float appearing after the word 'eigen' or 'eigenvalue'
        eig_matches = []
        for m in re.finditer(r"eigen(?:value)?", txt, re.I):
            # take following 200 chars and search for float
            span = txt[m.end():m.end()+200]
            fr = FLOAT_RE.search(span)
            if fr:
                eig_matches.append((m.start(), float(fr.group(1)), span))
        if not eig_matches:
            # fallback: search for any long float in file (last one)
